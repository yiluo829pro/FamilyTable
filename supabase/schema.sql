-- ============================================================
-- Family Table — Full Schema
-- Run this entire file in Supabase SQL Editor
-- ============================================================

-- ============================================================
-- STEP 1: CREATE ALL TABLES
-- (policies come after, so forward references are never an issue)
-- ============================================================
...
CREATE POLICY "dishes_select" ON public.dishes FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.tables WHERE id = dishes.table_id AND created_by = auth.uid()) OR
  EXISTS (SELECT 1 FROM public.table_members WHERE table_id = dishes.table_id AND user_id = auth.uid())
);
CREATE POLICY "dishes_insert" ON public.dishes FOR INSERT WITH CHECK (
  auth.uid() = created_by AND (
    EXISTS (SELECT 1 FROM public.tables WHERE id = dishes.table_id AND created_by = auth.uid()) OR
    EXISTS (SELECT 1 FROM public.table_members WHERE table_id = dishes.table_id AND user_id = auth.uid())
  )
);
CREATE POLICY "dishes_update" ON public.dishes FOR UPDATE USING (
  EXISTS (SELECT 1 FROM public.tables WHERE id = dishes.table_id AND created_by = auth.uid()) OR
  EXISTS (SELECT 1 FROM public.table_members WHERE table_id = dishes.table_id AND user_id = auth.uid())
);
CREATE POLICY "dishes_delete" ON public.dishes FOR DELETE USING (
  auth.uid() = created_by OR
  EXISTS (SELECT 1 FROM public.table_members WHERE table_id = dishes.table_id AND user_id = auth.uid() AND role = 'admin')
);

-- events
CREATE POLICY "events_select_auth" ON public.events FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.tables WHERE id = events.table_id AND created_by = auth.uid()) OR
  EXISTS (SELECT 1 FROM public.table_members WHERE table_id = events.table_id AND user_id = auth.uid())
);
CREATE POLICY "events_select_guest" ON public.events FOR SELECT USING (
  status IN ('live', 'voting_closed', 'menu_announced')
);
...
-- guest_votes (public — guests need no auth)
CREATE POLICY "votes_select" ON public.guest_votes FOR SELECT USING (true);
CREATE POLICY "votes_insert" ON public.guest_votes FOR INSERT WITH CHECK (true);
CREATE POLICY "votes_delete" ON public.guest_votes FOR DELETE USING (true);

-- guest_preferences (insert public; select admin only)
CREATE POLICY "prefs_insert" ON public.guest_preferences FOR INSERT WITH CHECK (true);
CREATE POLICY "prefs_select" ON public.guest_preferences FOR SELECT USING (
  EXISTS (
...
CREATE POLICY "wishlist_select" ON public.wishlist_items FOR SELECT USING (true);
CREATE POLICY "wishlist_insert" ON public.wishlist_items FOR INSERT WITH CHECK (true);
CREATE POLICY "wishlist_delete" ON public.wishlist_items FOR DELETE USING (true);

-- potluck_items (public)
CREATE POLICY "potluck_select" ON public.potluck_items FOR SELECT USING (true);
CREATE POLICY "potluck_insert" ON public.potluck_items FOR INSERT WITH CHECK (true);
CREATE POLICY "potluck_update" ON public.potluck_items FOR UPDATE USING (true);
CREATE POLICY "potluck_delete" ON public.potluck_items FOR DELETE USING (
  EXISTS (
    SELECT 1 FROM public.events e
    JOIN public.table_members tm ON tm.table_id = e.table_id
...
-- food_fund_bids (public)
CREATE POLICY "fund_select" ON public.food_fund_bids FOR SELECT USING (true);
CREATE POLICY "fund_insert" ON public.food_fund_bids FOR INSERT WITH CHECK (true);

-- drinks
CREATE POLICY "drinks_select" ON public.drinks FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.tables WHERE id = drinks.table_id AND created_by = auth.uid()) OR
  EXISTS (SELECT 1 FROM public.table_members WHERE table_id = drinks.table_id AND user_id = auth.uid())
);
CREATE POLICY "drinks_insert" ON public.drinks FOR INSERT WITH CHECK (
  auth.uid() = added_by AND (
    EXISTS (SELECT 1 FROM public.tables WHERE id = drinks.table_id AND created_by = auth.uid()) OR
    EXISTS (SELECT 1 FROM public.table_members WHERE table_id = drinks.table_id AND user_id = auth.uid())
  )
);
CREATE POLICY "drinks_update" ON public.drinks FOR UPDATE USING (
  EXISTS (SELECT 1 FROM public.tables WHERE id = drinks.table_id AND created_by = auth.uid()) OR
  EXISTS (SELECT 1 FROM public.table_members WHERE table_id = drinks.table_id AND user_id = auth.uid())
);
CREATE POLICY "drinks_delete" ON public.drinks FOR DELETE USING (
  auth.uid() = added_by OR
  EXISTS (SELECT 1 FROM public.table_members WHERE table_id = drinks.table_id AND user_id = auth.uid() AND role = 'admin')
);

-- misc_items
CREATE POLICY "misc_items_select" ON public.misc_items FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.tables WHERE id = misc_items.table_id AND created_by = auth.uid()) OR
  EXISTS (SELECT 1 FROM public.table_members WHERE table_id = misc_items.table_id AND user_id = auth.uid())
);
CREATE POLICY "misc_items_insert" ON public.misc_items FOR INSERT WITH CHECK (
  auth.uid() = added_by AND (
    EXISTS (SELECT 1 FROM public.tables WHERE id = misc_items.table_id AND created_by = auth.uid()) OR
    EXISTS (SELECT 1 FROM public.table_members WHERE table_id = misc_items.table_id AND user_id = auth.uid())
  )
);
CREATE POLICY "misc_items_update" ON public.misc_items FOR UPDATE USING (
  EXISTS (SELECT 1 FROM public.tables WHERE id = misc_items.table_id AND created_by = auth.uid()) OR
  EXISTS (SELECT 1 FROM public.table_members WHERE table_id = misc_items.table_id AND user_id = auth.uid())
);
CREATE POLICY "misc_items_delete" ON public.misc_items FOR DELETE USING (
  auth.uid() = added_by OR
...
-- collections
CREATE POLICY "collections_select_auth" ON public.collections FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.tables WHERE id = collections.table_id AND created_by = auth.uid()) OR
  EXISTS (SELECT 1 FROM public.table_members WHERE table_id = collections.table_id AND user_id = auth.uid())
);
CREATE POLICY "collections_select_public" ON public.collections FOR SELECT USING (privacy = 'shared');
...
CREATE POLICY "collections_update" ON public.collections FOR UPDATE USING (
  EXISTS (SELECT 1 FROM public.tables WHERE id = collections.table_id AND created_by = auth.uid()) OR
  EXISTS (SELECT 1 FROM public.table_members WHERE table_id = collections.table_id AND user_id = auth.uid())
);
CREATE POLICY "collections_delete" ON public.collections FOR DELETE USING (auth.uid() = created_by);
...
CREATE INDEX IF NOT EXISTS idx_prefs_event_id          ON public.guest_preferences(event_id);
CREATE INDEX IF NOT EXISTS idx_wishlist_event_id       ON public.wishlist_items(event_id);
...
CREATE INDEX IF NOT EXISTS idx_travel_moments_exp_id   ON public.travel_moments(experience_id);
